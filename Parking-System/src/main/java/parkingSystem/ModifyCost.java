...
/**
 * Servlet implementation class ModifyCost
 */
@WebServlet("/updateCost")
public class ModifyCost extends HttpServlet {
	private static final long serialVersionUID = 1L;
		...
		protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			try {
				...
				st.executeUpdate("update parkcost set cost='"+cost+"'");
				
				response.sendRedirect("ChangeCost.jsp?msg=done");
			}catch(Exception e){
				e.printStackTrace();
			}finally {
				try {
					...
					e.printStackTrace();
					response.sendRedirect("ChangeCost.jsp?msg=wrong");
				}
			}
		}
	}